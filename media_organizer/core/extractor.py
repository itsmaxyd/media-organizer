...
def extract_keyframes(video_path: Path, n_frames: int = 8) -> list[bytes]:
    ...
    try:
        ...
        logging.warning(f"Failed to extract keyframes from {video_path}: {e}")
        return []
...
def extract_audio(video_path: Path, tmp_dir: Path) -> Path | None:
    ...
    try:
        ...
        ffmpeg.input(str(video_path)).output(str(output_path), ac=1, ar=16000, f='wav').run(quiet=True)
        return output_path
    except Exception as e:
        logging.warning(f"Failed to extract audio from {video_path}: {e}")
        return None
